...
{
  var bonus;
  // Bei InstaGIB werden alle Spawnpunkte und die Waffen aus den Spielern entfernt
  if(FindObject(IGIB))
  {
    ...
    // Bonusspawnpunkte anpassen
    for(bonus in FindObjects(Find_ID(BSPN)))
    {
      if(GetLength(LocalN("objs", bonus)) == 1)
        bonus -> RemoveObject();
      else
      {
        bonus->RemoveBonus(BSKB); // Berserker entfernen
        bonus->RemoveBonus(HELB); // Heal entfernen
      }
    }
  }
